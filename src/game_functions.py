...
def get_number_rows(ai_settings, ship_height, alien_height):
	"""Determine the number o frows of aliens that fit on the screen."""
...
def change_fleet_direction(ai_settings, aliens):
	"""Drop the entier fleet and change the fleet's direction"""
	for alien in aliens.sprites():
...
def create_fleet(ai_settings, screen, ship, aliens):
	...
	# Create an alien and find the numbenr of aliens in a row.
	# Spacing between eanch alien is equal to one aliend width.
	alien = Alien(ai_settings, screen)
...
def check_keyup_events(event, ship):
	...
	if event.key == pygame.K_LEFT:
		# Stop moving the shop to the left
...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, bullets,
	...
	# Make the most recently drawn to screen visible.
	pygame.display.flip()